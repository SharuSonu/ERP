...
const PurcOrderForm = () => {
  ...
  const handleSubmit = async (values, actions) => {
    ...
    try {
      ...
      const response = await createPurcOrderVoucher(voucherData);
  ...
  const fetchVoucherNumber = async () => {
    try {
      ...
      const response = await axios.get(BASE_URL+'/purcorder_vouchers/last', {
        params: { companyName }
      });
  ...
  return (
    <Formik initialValues={initialValues} onSubmit={handleSubmit}>
      {({ values, setFieldValue, resetForm }) => {
        ...
        return (
          <Form className="purchaseOrder-form">
