...
const ledgerGstin = partyledgerdata.GSTIN || '';
        ...
        // Fetch item details
        const itemsResponse = await axios.get(BASE_URL + `/purcorder_inventory`, {
            params: {
                companyName: companyName,
        ...
        const ledgerResponse = await axios.get(BASE_URL + `/purcorder_ledger_entries`, {
            params: {
                companyName: companyName,
                vouchernumber: invoiceId
            }
        ...
        const ledgers = ledgerResponse.data;
         ...
         // Fetch hsn details
      const HsnResponse = await axios.get(BASE_URL + `/purcorder_hsn_tax_details`, {
          params: {
              companyName: companyName,
              vouchernumber: invoiceId
          }
  ...
  <script>
    window.onload = function() {
      window.print();
        ...
        console.error('Error fetching Purchase Order invoice details:', error);
        if (axios.isAxiosError(error)) {
            message.error(error.message); // Display Axios error message using Ant Design message component
        } else {
            message.error('Error fetching Purchase Order invoice details. Please try again.');
        }
    }
};
