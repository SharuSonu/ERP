...
const AllInvoicesForm = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(BASE_URL+'/sales_voucher', {
        params: {
  ...
  const handlePrintInvoice = async (invoiceId) => {
    ...
    //console.log("data : ",cmpdata);
      const response = await axios.get(BASE_URL+`/sales_voucher/${invoiceId}`, {
        params: {
          companyName: companyName
        }
      });
...
const ledgerGstin = partyledgerdata.GSTIN || '';
      ...
      if (response.data) {
        ...
        // Fetch item details
        const itemsResponse = await axios.get(BASE_URL+`/sales_inventory`, {
        params: {
        companyName: companyName,   
        ...
        const ledgerResponse = await axios.get(BASE_URL+`/sales_ledger_entries`, {
            params: {
            companyName: companyName,   
            vouchernumber: invoiceId
                 }
        ...
        const ledgers = ledgerResponse.data;  
             ...
             // Fetch hsn details
      const HsnResponse = await axios.get(BASE_URL + `/sales_hsn_tax_details`, {
        params: {
            companyName: companyName,
            vouchernumber: invoiceId
        }
...
function convertAmountToWords(amount) {
  ...
  return words;
        ...
        // Ask user which print format to use
        const isPOSPrint = window.confirm("Do you want to print in POS format? Click 'OK' for POS, 'Cancel' for Regular.");
