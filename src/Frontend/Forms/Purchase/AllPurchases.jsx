...
const AllPurchasesForm = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:5000/api/purchase_voucher', {
        params: {
  ...
  const handlePrintInvoice = async (invoiceId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/purchase_voucher/${invoiceId}`, {
        params: {
          companyName: companyName
        }
      });
