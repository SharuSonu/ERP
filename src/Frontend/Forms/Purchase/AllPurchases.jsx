...
import '../../../styles/Purchase/AllPurchases.css';
...
const AllPurchasesForm = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(BASE_URL+'/purchase_voucher', {
        params: {
  ...
  const handlePrintInvoice = async (invoiceId) => {
    try {
        ...
        const response = await axios.get(BASE_URL + `/purchase_voucher/${invoiceId}`, {
            params: {
                companyName: companyName
            }
        });
...
const ledgerGstin = partyledgerdata.GSTIN || '';
        ...
        // Fetch item details
        const itemsResponse = await axios.get(BASE_URL + `/purchase_inventory`, {
            params: {
                companyName: companyName,
        ...
        const ledgerResponse = await axios.get(BASE_URL + `/purchase_ledger_entries`, {
            params: {
                companyName: companyName,
                vouchernumber: invoiceId
            }
        ...
        const ledgers = ledgerResponse.data;
         ...
         // Fetch hsn details
      const HsnResponse = await axios.get(BASE_URL + `/purc_hsn_tax_details`, {
          params: {
              companyName: companyName,
              vouchernumber: invoiceId
          }
